...
namespace AI.GuardStates
{
    ...
    {
        private readonly float _baseWaitTime = 3.0f; // how long to wait before moving to new point
        private float _currentWaitTime; // current wait time
        private readonly float magDistance = 0.1f; // distance magnifier. This is used to determine how much distance affects next location 
        ...
        {
            if (guard.agent.autoBraking) guard.agent.autoBraking = false; // ensure guards don't slow down as they approach area
            ...
            _moveToPosition = GetNextRoamingPosition(guard, guard.transform.position, 5, -1); // Get the next position for the guard to navigate to
            guard.agent.SetDestination(_moveToPosition); // set destination as the new position vector

            guard.SetAgentSpeed(guard.initialAgentSpeed); // set the guard's speed (this changes when attacking so need to reset)

            _currentWaitTime = _baseWaitTime; // ensure the guard has a wait timer 
            guard.runCoroutine(LowerStaleness(1f, guard)); // create a coroutine handling the lowering of a region's staleness value. Testing required against multiple instances //todo
        }
        ...
        {
            while (true) // this will probably spawn multiple instances, boolean flag in manager to check this //todo
            {
                yield return new WaitForSeconds(waitTime); // wait for the allocated time
                go.ModifyValueAtCell(guard.transform.position, -7); // we add two every tick, so let's take that + 5 away.
            }
        }
        ...
        /// <summary>
        /// If collided, enter attack state. FOV ensures this so usually isn't needed, maybe if they get bumped this would run
        /// </summary>
        /// <param name="guard"></param>
        ...
        /// <param name="layermask">Navigable layermask https://docs.unity3d.com/540/Documentation/ScriptReference/NavMesh.SamplePosition.html</param>
        ...
        private Vector3 GetNextRoamingPosition(GuardStateMan guard, Vector3 origin, float dist, int layermask)
        {
            ...
            newVec += new Vector3(Random.Range(2,go._sz), 0, Random.Range(2,go._sz)) * .5f;
